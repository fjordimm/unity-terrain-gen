...
/*
If you want to make a major optimization for chunk generation,
	make it so that it doesn't have to regenerate the entire chunk for different LOD-edge-blendings and different LODs
*/

namespace Anaximer.OLDTerrainGeneration
{
	internal static class OLDChunkMeshGenerator
	{
		...
		public static Mesh MakeMesh(TerrainGene terrainGene, int size, float chunkScale, int xOff, int zOff)
		{
			...
			Vector3[] normals = Normals(verticesPre, xSize, zSize);
		...
		private static Vector3[] Normals(Vector3[] verticesPre, int xSize, int zSize)
		{
